...
When working on UI components or stories, always use the `giles-storybook-mcp` MCP server to inspect Storybook's component and story knowledge before answering or taking action.

- **CRITICAL: Never hallucinate component properties.** Before using any property on a design-system component—including common-sounding properties such as `shadow`—use the MCP tools to verify that the property is documented for that component.
- Query `docs-list` to find documented components when the docs toolset is available.
- Query `docs-show` for the relevant component to inspect its documented properties and examples. Query `docs-show-story` when the component entry does not provide enough detail.
- Use only properties explicitly documented or shown in example stories. A story name is not authoritative evidence for a prop name.
- If a property is not documented, do not infer it from naming conventions or another library. Check with the user before using it.
- Before creating or editing a story, query `get-storybook-story-instructions` and treat its response as the source of truth for story structure, props, and interaction coverage.
- After changing component visuals, styles, themes, design tokens, or stories, use `stories-preview` to inspect the affected stories.
- Use `test-run` to check stories when Storybook Test is configured. This project currently has no Storybook Test runner, so use the available Storybook build/smoke checks until that is added.
- Start Storybook with `npm run storybook`; the MCP endpoint is `http://localhost:6006/mcp`.

The Astro framework does not currently generate Storybook's components manifest, so `docs-list` and `docs-show` may report the docs toolset as unavailable. In that case, use the existing story files and the development/testing tools that are available; do not invent undocumented props.
